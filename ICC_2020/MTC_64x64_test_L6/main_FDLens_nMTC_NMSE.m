...
F_BB_q = exp(1i*2*pi*rand(N_RF_MS,N_MS_beam/N_MS_block));
W_BB_q = exp(1i*2*pi*rand(N_RF_BS,N_BS_beam/N_BS_block));
...
for i_PNR_dB = 1:length(PNR_dBs)
    ...
    for iter = 1:iterMax
        ...
        [H_v_CS_MTC_r, iter_num, Atom_sel] = OMP_delta(Q_CSM_MTC_r, y_MTC_r, dimension_of_dict_BS, dimension_of_dict_MS, sigma, Lp, 300);
        ...
        toc
        disp(['  nMTC, overhead = ' num2str(N_MS_beam) ', SNR = ' num2str(PNR_dBs(i_PNR_dB)) ', iter_max = ' num2str(iterMax) ', iter_now = ' num2str(iter) ', OMP_iter = ' num2str(iter_num) ...
...
if(save_flag)
    switch N_MS_beam
        case 16
            save NMSE_nMTC_o16_m15tom10_L6 NMSE_dB
        case 32
            save NMSE_nMTC_o32_m15_L6 NMSE_dB
        case 48
            save NMSE_nMTC_o48_m15tom10_L6 NMSE_dB
    end
end
%% Plot
% figure
