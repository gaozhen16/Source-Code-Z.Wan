...
%         tic
        ...
        Dic_set = -1+1/G_MS_r+(2-2/G_MS_r)*(0:(G_MS_r-1))/(G_MS_r-1);
        idx = [];
        for i = 1:Lp
            [~,idx1] = min(abs(sin(ele_MS(i))-Dic_set));
            [~,idx2] = min(abs(sin(azi_MS(i))-Dic_set));
            idx = [idx,sub2ind([G_MS_r,G_MS_r],idx2,idx1)];
        end
