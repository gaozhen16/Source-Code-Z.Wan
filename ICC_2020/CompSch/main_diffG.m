...
NMSE = zeros(1,length(G));
Time = zeros(1,length(G));
...
for i_G = 1:length(G)
    ...
    for iter = 1:iterMax
        ...
        H_v_CS_MTC_r = OMP_delta(Q_CSM_MTC_r, y_MTC_r, N_BS, dim_of_dict_MS, sigma, Lp);
...
%         H_up_OMP_MTC_r = H_v_CS_MTC_r;
        ...
        NMSE(i_G) = NMSE(i_G) + NMSE_temp;
        
        disp(['  MYsche, overhead = ' num2str(N_MS_beam) ', SNR = ' num2str(PNR_dBs) ', iter_max = ' num2str(iterMax) ', iter_now = ' num2str(iter) ...
            ' ,G = ' num2str(G(i_G)) ', NMSE_now = ' num2str(10*log10(NMSE_temp)) 'dB, NMSE_avg = ' num2str(10*log10(NMSE(i_G)/iter)) 'dB.']);
    end
    
    t2 = clock;
    
    Time(i_G) = etime(t2,t1)/iterMax;
    NMSE(i_G) = NMSE(i_G)/iterMax;
end
toc
% closematlabpool
...
disp('Finished all');
if(save_flag)
%     save NMSE_SNR30_G15to80_8w NMSE_dB
    save Time_SNR10_G20to80 Time
end
%% Plot
figure
plot(G,NMSE_dB,'-m^','LineWidth',1.5); grid on;
xlabel('G'),ylabel('NMSE [dB]')
axis tight