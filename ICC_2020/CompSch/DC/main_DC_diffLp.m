...
IndexN1 = 0:1:N1-1;
IndexN2 = 0:1:N2-1;
NMSE1 = zeros(N_iter,length(Lp));
NMSE2 = zeros(N_iter,length(Lp));
NMSE3 = zeros(N_iter,length(Lp));
cout2 = zeros(N_iter,length(Lp));
cout3 = zeros(N_iter,length(Lp));
...
%         NMSE2(iter,i_snr) = norm(Abhh - h_hat2,2)^2/norm(Abhh,2)^2;
        [h_hat3 ,cout3(iter,i_Lp)] = mySSD2_2(yy,W,N1,N2,L,maxNum);
        NMSE3(iter,i_Lp) = norm(Abhh - h_hat3,2)^2/norm(Abhh,2)^2;
    end
end
...
figure();
semilogy(Lp,NMSE1avg,'-ob','Linewidth',1.5);
hold on;
semilogy(Lp,NMSE2avg,'-*r','Linewidth',1.5);
semilogy(Lp,NMSE3avg,'-+k','Linewidth',1.5);
grid on;
