...
PNR_dBs = 10;
Lp = 2:6; %多径数
NMSE = zeros(1,length(Lp));

%% Channel Estimation 
tic
% startmatlabpool(length(PNR_dBs))
for i_Lp = 1:length(Lp)
    sigma2 = 10^(-(PNR_dBs/10));
    sigma = sqrt(sigma2);
    for iter = 1:iterMax
        H_up = mmWave_uplink_channel_FDLens(D_MS, D_BS, ele_max_MS, azi_max_MS, ele_max_BS, azi_max_BS, Lp(i_Lp), 0);    
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Original below
        Q_CSM_MTC_r = kron(F_MTC.'*conj(A_MS_D_r),W_MTC');
        H_v_CS_MTC_r = OMP_delta(Q_CSM_MTC_r, y_MTC_r, N_BS, dim_of_dict_MS, sigma, Lp(i_Lp));
        H_up_OMP_MTC_r = H_v_CS_MTC_r * A_MS_D_r';
...
%         H_up_OMP_MTC_r = H_v_CS_MTC_r;
        ...
        NMSE(i_Lp) = NMSE(i_Lp) + NMSE_temp;
        
%         toc
        disp(['  MYsche, overhead = ' num2str(N_MS_beam) ', SNR = ' num2str(PNR_dBs) ', iter_max = ' num2str(iterMax) ', iter_now = ' num2str(iter) ...
            ' ,Lp = ' num2str(Lp(i_Lp)) ', NMSE_now = ' num2str(10*log10(NMSE_temp)) 'dB, NMSE_avg = ' num2str(10*log10(NMSE(i_Lp)/iter)) 'dB.']);
    end
    NMSE(i_Lp) = NMSE(i_Lp)/iterMax;
%     toc
    disp(['Finished ',num2str(i_Lp),'/', num2str(length(Lp)) ' , NMSE_OMP = ' num2str(10*log10(NMSE(i_Lp))) 'dB']);
end
toc
% closematlabpool
...
disp('Finished all');
if(save_flag)
    save NMSE_Redu_128_G40_SNR10 NMSE_dB
end
%% Plot
figure
plot(Lp,NMSE_dB,'-m^','LineWidth',1.5); grid on;
